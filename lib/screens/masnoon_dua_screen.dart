...
/// Masnoon Dua Screen
/// Displays Masnoon Duas with lazy loading pagination
class MasnoonDuaScreen extends StatefulWidget {
  const MasnoonDuaScreen({super.key});
...
class _MasnoonDuaScreenState extends State<MasnoonDuaScreen> {
  ...
  Future<void> _loadDuas() async {
    try {
      final duas = await _duaService.getDuasPaginated(
        page: 0,
        pageSize: _pageSize,
      );
      if (mounted) {
        setState(() {
          _duas = duas;
          _currentPage = 0;
          _hasMoreData = duas.length >= _pageSize;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _loadMoreDuas() async {
    ...
    try {
      ...
      final newDuas = await _duaService.getDuasPaginated(
        page: nextPage,
        pageSize: _pageSize,
      );
      ...
      if (mounted) {
        setState(() {
          _duas.addAll(newDuas);
          _currentPage = nextPage;
          _hasMoreData = newDuas.length >= _pageSize;
  ...
  Widget _buildDuasList(bool isDarkMode) {
    return ListView.builder(
      controller: _scrollController,
      physics: const BouncingScrollPhysics(),
      padding: const EdgeInsets.symmetric(vertical: 12),
      itemCount: _duas.length + (_hasMoreData ? 1 : 0),
      itemBuilder: (context, index) {
        if (index >= _duas.length) {
          return _buildLoadingIndicator(isDarkMode);
        }
        return DuaCard(dua: _duas[index], isDarkMode: isDarkMode);
      },
    );
  }
  ...
  Widget _buildLoadingState(bool isDarkMode) {
    return ListView.builder(
      ...
      padding: const EdgeInsets.symmetric(vertical: 12),
      itemBuilder: (context, index) {
        return DuaCardSkeleton(isDarkMode: isDarkMode);
      },
    );
  }
